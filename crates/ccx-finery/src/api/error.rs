...
pub struct FineryApiErrorInfo {
    #[serde(default)]
    pub message: SmartString<104>,
}

// TODO: maybe split this big enum into separate parts?
/// [source](https://www.finery.io/docs/developers/apiv4/en/#label-list)
///
...
pub enum FineryApiError {
    ...
    ApiWithdrawDisabled(FineryApiErrorInfo),

    /// Invalid withdraw ID
    #[error("Invalid withdraw ID {0:?}")]
    #[serde(rename = "INVALID_WITHDRAW_ID")]
    InvalidWithdrawId(FineryApiErrorInfo),
    ...
    InvalidPrecision(FineryApiErrorInfo),

    /// Invalid currency
    #[error("Invalid currency {0:?}")]
    #[serde(rename = "INVALID_CURRENCY")]
    InvalidCurrency(FineryApiErrorInfo),

    /// Invalid currency pair
    #[error("Invalid currency pair {0:?}")]
    #[serde(rename = "INVALID_CURRENCY_PAIR")]
    InvalidCurrencyPair(FineryApiErrorInfo),
    ...
    TooManyCurrencyPairs(FineryApiErrorInfo),
    ...
    TooManyOrders(FineryApiErrorInfo),
    ...
    ClientError(FineryApiErrorInfo),
}
...
mod tests {
    ...
    fn test_failed_response() {
        let sample = FineryApiErrorInfo {
            message: "Invalid signature".to_fmt(),
        };

        let resp = serde_json::from_str::<FineryApiError>(FAILED_RESPONSE).unwrap();

        assert_eq!(resp, FineryApiError::InvalidSignature(sample));
    }
}
