...
use super::FineryClient;
use super::meta::{FineryError, FineryResponseMeta};
    ...
    T: Request,
{
    ...
    async fn send(self, client: &FineryClient) -> FineryResult<T::Response> {
        ...
        if let Some(sign) = self.sign_data {
            ...
            // https://www.finery.io/docs/developers/apiv4/#apiv4-signed-request-requirements
            request = request
...
async fn handle_response<T>(resp: reqwest::Response) -> Result<T, FineryError>
where
    T: Response,
{
    if resp.status().is_success() {
        ...
        let err = resp.json::<FineryApiError>().await?;

        tracing::error!(?err);

        Err(FineryError::Api(err))
    }
}

async fn handle_response_with_meta<T>(resp: reqwest::Response) -> FineryResult<T>
where
    T: Response,
{
    let meta = FineryResponseMeta::from_response(&resp);
